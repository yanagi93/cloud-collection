...
export default function ConfirmPage() {
    ...
    const handleNext = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/backend/cloud-photos", {
                method: "POST",
                headers: {
            ...
            if (!res.ok) {
                ...
                setMessage(`アップロード失敗: ${data.message ?? "エラーが発生しました"}`);
                return;
            }
            ...
            sessionStorage.setItem("uploadedCloudPhoto", JSON.stringify(data));
        } catch (error) {
            console.error(error);
            setMessage("通信エラーです。バックエンド起動またはCORS設定を確認してください。");
        } finally {
    ...
    return (
        <main className="min-h-screen bg-sky-300 flex flex-col items-center justify-center p-4 relative overflow-hidden select-none">
            ...
            <div className="w-full max-w-md z-10">
                <Card className="p-4 text-center bg-white" borderColor="#000000" shadowColor="#222222">
                    ...
                    {uploadResult && (
                        <div className="mb-4 border-2 border-black bg-green-50 px-3 py-2 text-xs text-gray-800 text-left break-all">
                            <p className="font-bold mb-1">バックエンドレスポンス</p>
                            <p>photoId: {uploadResult.id ?? "idなし"}</p>
                            <p>status: {uploadResult.status ?? "statusなし"}</p>
                        </div>
                    )}
